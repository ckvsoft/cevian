...
class Validate
{
    ...
    public function length($value, $param)
    {
        ...
        if (!is_array($param) || count($param) > 2)
            throw new \ckvsoft\CkvException(__CLASS__ . ': Length Parameter must be an array of 1 (exact) or 2 (min/max).');
    ...
    {
        if (!is_int($param))
            throw new \ckvsoft\CkvException(__CLASS__ . ": must supply an integer: $method");
    ...
    public function eqany($value, $param = array(), $caseSensitive = true)
    {
        ...
        if (!is_array($param))
            throw new \ckvsoft\CkvException(__CLASS__ . ': matchAny $param must be any array');
    ...
    private function _ageCalc($dob)
    {
        ...
        if (!preg_match("/^([0-9]{4})-([0-9]{2})-([0-9]{2})$/", $dob)) {
            throw new \ckvsoft\CkvException('date format must be in YYYY-MM-DD');
        }
